...
class mrp_property_group(models.Model):
    """
...
class mrp_property(models.Model):
    """
    Properties of mrp.
    """
    _name = 'mrp.property'
...
class mrp_workcenter(models.Model):
    ...
    def on_change_product_cost(self, product_id):
        value = {}

        if product_id:
            cost = self.env['product.product'].browse(product_id)
...
class mrp_routing(models.Model):
    ...
    _defaults = {
        'active': lambda *a: 1,
        'company_id': lambda self, cr, uid, context: self.env['res.company']._company_default_get('mrp.routing')
    }
...
class mrp_routing_workcenter(models.Model):
    ...
    routing_id = fields.Many2one('mrp.routing', 'Parent Routing', index=True, ondelete='cascade'
...
class mrp_bom(models.Model):
    ...
    product_qty = fields.Float('Product Quantity', required=True, digits=dp.get_precision('Product Unit of Measure'))
    ...
    _defaults = {
        ...
        'company_id': lambda self, cr, uid, c: self.env['res.company']._company_default_get('mrp.bom'),
    }
    _order = "sequence"
    ...
    def _bom_find(self, product_tmpl_id=None, product_id=None, properties=None):
        ...
        ids = self.search(domain, order='sequence, product_id')
        ...
        for bom in self.env['mrp.bom'].browse(ids):
        ...
        for bom_line_id in bom.bom_line_ids:
            ...
            if (not bom_id) or (self.browse(bom_id).type != "phantom"):
                result.append(self._prepare_consume_line(
                    cr, uid, bom_line_id, quantity))
            else:
                all_prod = [bom.product_tmpl_id.id] + (previous_products or [])
                bom2 = self.browse(bom_id)
                # We need to convert to units/UoM of chosen BoM
                factor2 = uom_obj._compute_qty(bom_line_id.product_uom.id, quantity, bom2.product_uom.id)
    ...
    def copy_data(self, id, default=None):
        if default is None:
            default = {}
        bom_data = self.read(id, [])
        default.update(name=_("%s (copy)") % (bom_data['display_name']))
        return super(mrp_bom, self).copy_data(id, default)

    def onchange_uom(self, product_tmpl_id, product_uom):
        res = {'value': {}}
    ...
    def unlink(self):
        if self.env['mrp.production'].search([('bom_id', 'in', ids), ('state', 'not in', ['done', 'cancel'])]):
            raise UserError(_('You can not delete a Bill of Material with running manufacturing orders.\nPlease close or cancel it first.'))
        return super(mrp_bom, self).unlink(ids)
...
class mrp_bom_line(models.Model):
    ...
    def _get_child_bom_lines(self, field_name, arg):
    ...
    bom_id = fields.Many2one('mrp.bom', 'Parent BoM', ondelete='cascade', index=True, required=True)
    attribute_value_ids = fields.Many2many('product.attribute.value', string='Variants', help="BOM Product Variants needed form apply this line.")
        'child_line_ids': fields.One2many(compute="_get_child_bom_lines", relation="mrp.bom.line", string="BOM lines of the referred bom", )
    ...
    def create(self, values):
        ...
        if 'product_id' in values and not 'product_uom' in values:
            values['product_uom'] = product_obj.browse(values.get('product_id')).uom_id.id
        return super(mrp_bom_line, self).create(values)
...
class mrp_production(models.Model):
    ...
    def _production_calc(self, prop, unknow_none):
    ...
    def _get_workcenter_line(self):
        result = {}
        for line in self.env['mrp.production.workcenter.line'].browse(ids):
    ...
    def _src_id_default(self):
        try:
            ...
            self.env['stock.location'].check_access_rule([location_id], 'read')
    ...
    def _get_progress(self, name, arg):
        """ Return product quantity percentage """
        result = dict.fromkeys(ids, 100)
        for mrp_production in self.browse(ids):
    ...
    def _moves_assigned(self, name, arg):
    ...
    priority = fields.Selection([('0', 'Not urgent'), ('1', 'Normal'), ('2', 'Urgent'), ('3', 'Very Urgent')], 'Priority'
            index=True, readonly=True, states=dict.fromkeys(['draft', 'confirmed'], [('readonly', False)])),

        'product_id': fields.Many2one('product.product', 'Product', required=True, readonly=True, states={'draft': [('readonly', False)]}, 
                                      domain=[('type', 'in', ['product', 'consu'])]),
    product_qty = fields.Float('Product Quantity', digits=dp.get_precision('Product Unit of Measure'), required=True, readonly=True, states={'draft': [('readonly', False)]})
    ...
    date_planned = fields.Datetime('Scheduled Date', required=True, index=1, readonly=True, states={'draft': [('readonly', False)]}, copy=False)
    date_start = fields.Datetime('Start Date', index=True, readonly=True, copy=False)
    date_finished = fields.Datetime('End Date', index=True, readonly=True, copy=False)
    ...
    workcenter_lines = fields.One2many('mrp.production.workcenter.line', 'production_id', 'Work Centers Utilisation'
        ...
        'hour_total': fields.Float(compute="_production_calc", string='Total Hours', store={
            _name: (lambda self, cr, uid, ids, c={}: ids, ['workcenter_lines'], 40),
            'mrp.production.workcenter.line': (_get_workcenter_line, ['hour', 'cycle'], 40),
        }),
        'cycle_total': fields.Float(compute="_production_calc", string='Total Cycles', store={
            _name: (lambda self, cr, uid, ids, c={}: ids, ['workcenter_lines'], 40),
            'mrp.production.workcenter.line': (_get_workcenter_line, ['hour', 'cycle'], 40),
        }),
    user_id = fields.Many2one('res.users', 'Responsible')
    company_id = fields.Many2one('res.company', 'Company', required=True)
    ready_production = fields.Boolean(compute="_moves_assigned", string="Ready for production", store={'stock.move': (_mrp_from_move, ['state'], 10)})
    ...
    _defaults = {
        ...
        'name': lambda self, cr, uid, context: self.env['ir.sequence'].next_by_code('mrp.production') or '/',
        'company_id': lambda self, cr, uid, c: self.env['res.company']._company_default_get('mrp.production'),
    ...
    def create(self, values):
        ...
        if 'product_id' in values and not 'product_uom' in values:
            values['product_uom'] = product_obj.browse(values.get('product_id')).uom_id.id
        return super(mrp_production, self).create(values)
    ...
    def unlink(self):
        ...
        return super(mrp_production, self).unlink(ids)

    def location_id_change(self, src, dest):
    ...
    def product_id_change(self, product_id, product_qty=0):
        ...
        bom_obj = self.env['mrp.bom']
        product = self.env['product.product'].browse(product_id)
        bom_id = bom_obj._bom_find(product_id=product.id, properties=[])
        routing_id = False
        if bom_id:
    ...
    def _prepare_lines(self, production, properties=None):
        ...
        factor = uom_obj._compute_qty(production.product_uom.id, production.product_qty, bom_point.product_uom.id)
    ...
    def _action_compute_lines(self, properties=None):
        ...
        for production in self.browse(ids):
            ...
            prod_line_obj.unlink([line.id for line in production.product_lines])
            #unlink workcenter_lines
            workcenter_line_obj.unlink([line.id for line in production.workcenter_lines])
            ...
            for line in results2:
                line['production_id'] = production.id
                workcenter_line_obj.create(line, context)
        return results
    ...
    def action_compute(self, properties=None):
        ...
        """
        return len(self._action_compute_lines(ids, properties=properties))
    ...
    def action_cancel(self):
        ...
        for production in self.browse(ids):
            ...
            procs = proc_obj.search([('move_dest_id', 'in', [x.id for x in production.move_lines])])
            if procs:
                proc_obj.cancel(procs)
            move_obj.action_cancel([x.id for x in production.move_lines])
        self.write(ids, {'state': 'cancel'})
        # Put related procurements in exception
        proc_obj = self.env["procurement.order"]
        procs = proc_obj.search([('production_id', 'in', ids)])
        if procs:
            for proc in procs:
                proc_obj.message_post(proc, body=_('Manufacturing order cancelled.'))
            proc_obj.write(procs, {'state': 'exception'})
        return True
    ...
    def action_ready(self):
        ...
        for production in self.browse(ids):
            ...
            if production.move_prod_id and production.move_prod_id.location_id.id != production.location_dest_id.id:
                move_obj.write([production.move_prod_id.id],
                        {'location_id': production.location_dest_id.id})
        return True

    def _compute_costs_from_production(self):
        """ Generate workcenter costs in analytic accounts"""
        for production in self.browse(ids):
            total_cost = self._costs_generate(production)

    def action_production_end(self):
        """ Changes production state to Finish and writes finished date.
        @return: True
        """
        self._compute_costs_from_production(ids, context)
        write_res = self.write(ids, {'state': 'done', 'date_finished': time.strftime('%Y-%m-%d %H:%M:%S')})
    ...
    def _calculate_qty(self, production, product_qty=0.0):
        ...
        for product_id in scheduled_qty.keys():
            ...
            for move in production.move_lines:
                ...
                quants = quant_obj.quants_get_preferred_domain(q, move, domain=[('qty', '>', 0.0)],
    ...
    def _calculate_total_cost(self, total_consume_moves):
        total_cost = 0
        for consumed_move in self.env['stock.move'].browse(total_consume_moves):
            total_cost += sum([x.inventory_value for x in consumed_move.quant_ids if x.qty > 0])
        return total_cost

    def _calculate_workcenter_cost(self, production_id):
    ...
    def action_produce(self, production_id, production_qty, production_mode, wiz=False):
        ...
        if production_mode in ['consume', 'consume_produce']:
            ...
            for consume in consume_lines:
                ...
                for raw_material_line in production.move_lines:
                    ...
                    stock_mov_obj.action_consume([raw_material_line.id], consumed_qty, raw_material_line.location_id.id,
                ...
                if not float_is_zero(remaining_qty, precision_digits=precision):
                    ...
                    stock_mov_obj.write([extra_move_id], {'restrict_lot_id': consume['lot_id'],
                                                                    'consumed_for': main_production_move})
                    stock_mov_obj.action_done([extra_move_id])
        ...
        if production_mode == 'consume_produce':
            ...
            for produce_product in production.move_created_ids:
                ...
                if is_main_product:
                    ...
                    production_cost = self._calculate_workcenter_cost(production_id)
                ...
                if is_main_product and price_unit:
                    stock_mov_obj.write([produce_product.id], {'price_unit': price_unit})
                new_moves = stock_mov_obj.action_consume([produce_product.id], qty,
                                                         location_id=produce_product.location_id.id, restrict_lot_id=lot_id)
                stock_mov_obj.write(new_moves, {'production_id': production_id})
                ...
                if not float_is_zero(remaining_qty, precision_digits=precision):
                    ...
                    if is_main_product:
                        stock_mov_obj.write([extra_move_id], {'price_unit': price_unit})
                    stock_mov_obj.action_confirm([extra_move_id])
                    stock_mov_obj.action_done([extra_move_id])

        self.message_post(production_id, body=_("%s produced") % self._description)
        ...
        if not production.move_created_ids and production.move_lines:
            stock_mov_obj.action_cancel([x.id for x in production.move_lines])

        self.signal_workflow([production_id], 'button_produce_done')
        return True
    ...
    def action_in_production(self):
        ...
        @return: True
        """
        return self.write(ids, {'state': 'in_production', 'date_start': time.strftime('%Y-%m-%d %H:%M:%S')})

    def consume_lines_get(self, *args):
        res = []
        for order in self.browse(ids):
    ...
    def _make_production_produce_line(self, production):
        ...
        procurement = procs and\
            proc_obj.browse(procs[0]) or False
        data = {
        ...
        return stock_move.action_confirm([move_id])[0]
    ...
    def _get_raw_material_procure_method(self, product, location_id=False, location_dest_id=False):
        ...
        if location_id and location_dest_id:
            ...
            if pulls:
                return pull_obj.browse(pulls[0]).procure_method

        try:
            mto_route = warehouse_obj._get_mto_route(context=context)
        except:
    ...
    def _create_previous_move(self, move_id, product, source_location_id, dest_location_id):
        ...
        if code == 'outgoing':
            ...
            check_loc = dest_loc
        wh = loc_obj.get_warehouse(check_loc)
        domain = [('code', '=', code)]
        if wh: 
    ...
    def _make_consume_line_from_data(self, production, product, uom_id, qty):
        ...
        move_id = stock_move.create({
            ...
            'origin': production.name,
            'warehouse_id': loc_obj.get_warehouse(production.location_src_id),
        ...
        if prev_move:
            ...
            stock_move.action_confirm([prev_move])
        return move_id

    def _make_production_consume_line(self, line):
        return self._make_consume_line_from_data(line.production_id, line.product_id, line.product_uom.id, line.product_qty)

    def action_confirm(self):
        """ Confirms production order.
        @return: Newly generated Shipment Id.
        """
        user_lang = self.env['res.users'].browse([uid]).partner_id.lang
        context = dict(context, lang=user_lang)
        uncompute_ids = filter(lambda x: x, [not x.product_lines and x.id or False for x in self.browse(ids)])
        self.action_compute(uncompute_ids)
        for production in self.browse(ids):
            self._make_production_produce_line(production)
            stock_moves = []
            ...
            if stock_moves:
                self.env['stock.move'].action_confirm(stock_moves)
    ...
    def action_assign(self):
        ...
        move_obj = self.env["stock.move"]
        for production in self.browse(ids):
            move_obj.action_assign([x.id for x in production.move_lines])
            if self.env['mrp.production'].test_ready([production.id]):
    ...
    def force_production(self, *args):
        ...
        move_obj = self.env['stock.move']
        for order in self.browse(ids):
            move_obj.force_assign([x.id for x in order.move_lines])
            if self.env['mrp.production'].test_ready([order.id]):
...
class mrp_production_workcenter_line(models.Model):
    ...
    production_id = fields.Many2one('mrp.production', 'Manufacturing Order'
            track_visibility='onchange', index=True, ondelete='cascade', required=True),
...
class mrp_production_product_line(models.Model):
    ...
    name = fields.Char('Name', required=True)
    product_id = fields.Many2one('product.product', 'Product', required=True)
    product_qty = fields.Float('Product Quantity', digits=dp.get_precision('Product Unit of Measure'), required=True)
    product_uom = fields.Many2one('product.uom', 'Product Unit of Measure', required=True)
