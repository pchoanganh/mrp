...
class TestMrpMulticompany(common.TransactionCase):
    ...
    def setUp(self):
        ...
        group_user_id = self.registry('ir.model.data').xmlid_to_res_id('base.group_user')
        group_stock_manager_id = self.registry('ir.model.data').xmlid_to_res_id('stock.group_stock_manager')
        company_2_id = self.registry('ir.model.data').xmlid_to_res_id('stock.res_company_1')
    ...
    def test_00_multicompany_user(self):
        ...
        defaults = self.stock_location.default_get(['location_id', 'location_dest_id', 'type'], context)
        for field in fields:
            if defaults.get(field):
                try:
                    self.stock_location.check_access_rule([defaults[field]], 'read', context)
